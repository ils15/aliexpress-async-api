...
class ShippingEndpoint(BaseEndpoint):
    ...
    async def get_shipping_info(
        self, product_id: int, country_code: str = "US", **kwargs
    ) -> ShippingInfo:
        """
        ...
        response = await self.request("aliexpress.affiliate.shipping.get", params)
    ...
    async def get_sku_detail(
        self, product_id: int, skill_id: str = "", **kwargs
    ) -> List[SKUInfo]:
        """
        ...
        response = await self.request("aliexpress.affiliate.sku.detail.get", params)
