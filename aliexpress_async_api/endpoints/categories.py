...
class CategoriesEndpoint(BaseEndpoint):
    ...
    async def get_categories(self, language: str = "en", **kwargs) -> List[Category]:
        """
        ...
        response = await self.request("aliexpress.affiliate.category.get", params)
    ...
    async def get_featured_promo(
        ...
        response = await self.request("aliexpress.affiliate.promo.featured.get", params)

        return response.get("promos", {})

    async def get_featured_promo_products(
        self, promo_id: str, page_no: int = 1, page_size: int = 100, **kwargs
    ) -> List[Dict[str, Any]]:
        """
        ...
        response = await self.request("aliexpress.affiliate.promo.product.list", params)

        products = response.get("products", {}).get("product", [])
        return (
            products if isinstance(products, list) else [products] if products else []
        )
