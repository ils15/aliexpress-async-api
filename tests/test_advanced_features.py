...
from aliexpress_async_api.utils.webhooks import WebhookPayload, WebhookManager, WebhookEvent
...
class TestWebhookManager:
    ...
    def test_webhook_unregistration(self):
        ...
        assert "http://example.com/webhook" not in manager.webhooks.get("order.completed", [])
    ...
    def test_webhook_unregister_all_events(self):
        ...
        assert "http://example.com/webhook" not in manager.webhooks.get("order.completed", [])
        assert "http://example.com/webhook" not in manager.webhooks.get("product.found", [])
    ...
    async def test_webhook_dispatch_to_subscribers(self):
        ...
        # Mock aiohttp
        with patch('aliexpress_async_api.utils.webhooks.aiohttp.ClientSession') as mock_session:
